...
# 创建VotingRegressor模型进行模型融合
voting_model = VotingRegressor(estimators=[('xgb', best_xgb_model), ('rf', best_rf_model)])
# 交叉验证评估模型性能
cv_scores_voting = cross_val_score(voting_model, X, y, cv=5)
print("Voting交叉验证得分:", cv_scores_voting)
print("Voting平均交叉验证得分:", np.mean(cv_scores_voting))
...
# 输出Voting模型的性能指标
print("Voting测试集R方（R²）:", r2_voting)
print("Voting测试集均方误差（MSE）:", mse_voting)
print("Voting测试集平均绝对误差（MAE）:", mae_voting)
...
# 使用训练好的Voting模型进行未来一年的目标变量预测
future_target_voting = voting_model.predict(future_features)

# 打印未来一年的目标变量预测结果
print("Voting未来一年的目标变量预测结果:", future_target_voting)